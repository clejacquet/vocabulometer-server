...
module.exports = (mongoose, models) => {
	...
	Object.keys(models.languages).forEach(language => {
        ...
        const textSchema = new mongoose.Schema({
            ...
            collection: languageModel.format('texts')
		});

        textSchema.statics.loadAndCreateTexts = function (texts, cb) {
            const bodies = texts.map(text => text.body);

            models.nlp.compute(bodies, languageModel, (err, computedBodies) => {
                if (err) {
                    return cb(err);
                }
        ...
        textSchema.statics.loadAndModifyText = function (id, text, cb) {
            models.nlp.compute([text], languageModel, (err, texts) => {
                if (err) {
                    return cb(err);
                }
        ...
        textModels[language] = mongoose.model(languageModel.format('Text'), textSchema);
	});
